...
## 機能要件
- ~~MUST: Googleアカウントでログインできる（OAuthソーシャルログイン）~~ → **スコープ外**（ADR-0014参照）
...
- MUST: Classroom APIで講座一覧を自動取得し、講座選択に利用する
- MUST: 管理者が対象講座をClassroomのCourse IDで指定できる
- MUST: 対象講座の表示/非表示を管理者が設定できる
- MUST: 講座同期の対象範囲はドメイン全体を前提とする
...
- MUST: 管理画面でCourse IDを登録し、対象講座の有効/表示を切り替えできる
- MUST: 管理画面で講座同期の状態（最終同期/エラー）を確認できる
...
## 非機能要件
- セキュリティ: OAuth/管理者権限の適切な管理、監査ログ保管
