...
- **再入室禁止**: 退室後は同一講座への再入室不可、管理者がリセット可能（ADR-0026）
- **ユーザー作成時のallowed_emails自動追加**: 受講者管理で登録後すぐにログイン可能（ADR-0027）

全ADRは`docs/decisions.md`を参照。

## 機能実装時の注意（認証・アクセス制御）

- **ユーザー作成時は`allowed_emails`への追加も必須**: `POST /admin/users`でユーザーを作成する際、`allowed_emails`にも自動追加すること。これがないとユーザーはログインできない
- **E2Eフローの検証**: 「管理者が登録 → ユーザーがログイン → 機能を使用」の一連の流れが動作することを確認
- **ADRとの整合性**: 認証に関わるADR（ADR-0016, ADR-0017）に記載された仕様と、既存の実装が整合しているか確認
...
- **再入室禁止機能**（ADR-0026）:
  - 退室後は同一講座への再入室をブロック（403 already_completed）
  - 講座一覧・セッション画面に「受講済み」バッジ表示
  - 受講済みでもGoogle Classroomリンクは利用可能
  - 管理者がセッションを削除してリセット可能
